...
const LoginPage = () =>{
	...
	const form = useForm<z.infer<typeof FormSchema>>({ mode: 'onChange', resolver: zodResolver(FormSchema), defaultValues: {email:'', password:''},})
		...
		<Form {...form}> 
			<form onChange={()=>{if(submitError)setSubmitError('');}} onSubmit={form.handleSubmit(onSubmit)} className='w-full sm:justify-center sm:w-[400px] space-y-6 flex flex-col'>
				<Link href='/' className="w-full flex justify-left items-center">
					<Image src={Logo} alt='bsamaritan Logo' width={50} height={50}/>
					<span className='font-semibold dark:text-white text-4xl first-letter:ml-2'> Bsamaritan. </span>
				</Link>
                ...
                <Button type='submit' disabled={isLoading} className='w-full p-6' size='lg'>
